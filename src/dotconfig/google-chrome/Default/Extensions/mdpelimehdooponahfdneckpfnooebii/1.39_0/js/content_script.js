    ...
    function (request, sender, sendResponse) {
        ...
        if (request.contentScriptQuery == 'currentGroup') {
            ...
            chrome.storage.local.get("licenseStatus", function (res) {
                if (res.licenseStatus.key || res.licenseStatus.TRIAL_USAGE_COUNT < 3) {
...
function injectJS(url) {
    ...
    var html = `<div id="loadingAnimation" class="loading-spinner">
                    <video src='${chrome.extension.getURL('/video/green-spinner.mp4')}' autoplay muted loop height='70px'></video>
                </div><style>.loading-spinner {z-index:9999;position: fixed;top:50%;left: 50%;transform: translate(-50%,-50%);display:none;}</style>
...
WAXP = (function () {
    ...
    var CONTACTS_WITHOUT_PHONE_NUMBER = new Set();
