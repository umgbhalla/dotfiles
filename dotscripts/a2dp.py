        ...
        for l in self.output.splitlines():
            if fail_expression and re.search(fail_expression, l, re.IGNORECASE):
                raise SubprocessError(
                    'Expression "%s" failed with fail pattern: "%s"' % (l, fail_expression))
...
async def set_profile(device_id, profile):
    ...
    try:
        ...
        print('Invalid profile: %s, please select one one of a2dp or hsp.' %
...
async def main(args):
    ...
    transport, protocol = await asyncio.get_event_loop().subprocess_exec(
        lambda: BluetoothctlProtocol(
            exit_future, echo=args.echo), 'bluetoothctl'
    )
    ...
    try:
        ...
        if device_id is None:
            print(
                'It seems device: %s is not connected yet, trying to connect.' % ':'.join(mac))
